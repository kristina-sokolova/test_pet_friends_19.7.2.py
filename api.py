...
class PetFriends:
    ...
    def create_pet_simple_without_photo(self, auth_key: json, name: str, animal_type: str, age: str) -> json:
    ...
    def add_photo_of_pet(self, auth_key: json, pet_id: str, pet_photo: str) -> json:
        """Метод делает post запрос к API сервера и добавляет новое фото питомца (определенного питомца, найденного по
        pet_id), возвращает код статуса запроса и результат в формате json с информацией о животном"""
        data = MultipartEncoder(
            fields={
